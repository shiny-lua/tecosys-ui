...
import { getPrompts } from "@/services/dispatch/prompts-dispatch";
...
export default function ChatInput() {
  ...
  // Initialize Community_prompts as an empty array
  const [Community_prompts, setCommunity_Prompts] = useState([]);

  // Function to fetch and update Community_prompts
  const fetchAndFilterPrompts = async () => {
    try {
      // Fetch prompts from the API
      const response = await getPrompts();

      // Transform the fetched prompts to match the desired structure
      const transformedPrompts = response.prompts.map((prompt) => ({
        name: prompt.title,
        content: prompt.content, // Avoid default sentence
      }));

      // Update Community_prompts with the transformed data
      setCommunity_Prompts(transformedPrompts);
    } catch (error) {
